...
class Apiv4Controller extends Controller
{
	...
	public function getunitlist(Request $request) 
    {
		...
		if(isset($unit_lists)){
			foreach($unit_lists as $unit){
				...
				$data['unit'] = isset($unit->addunitinfo)?$unit->addunitinfo->unit:null;
				$data['building'] = isset($unit->addubuildinginfo)?$unit->addubuildinginfo->building:null;
				$data['property'] = isset($unit->addpropinfo)?$unit->addpropinfo->company_name:null;
	...
	public function switchunit(Request $request) 
    {
		...
		if(isset($unit_info)){
			user::where('id',$user)->update( array( 'account_id' =>$unit_info->property_id,'unit_no' =>$unit_info->unit_id,'building_no' =>$unit_info->building_id,'user_info_id'=>$unit_info->user_info_id,'role_id'=>$unit_info->role_id));
			$userObj = User::find($user); 
	...
	public function FacialRegPicOption(Request $request)
   {
	...
	public function dashboardmenu(Request $request) {
		...
		$favmenus = UserFavMenu::where('user_id', $user)->where('account_id', $property)->get();
	...
	public function delete_favmenu(Request $request) {
		...
		UserFavMenu::where('user_id', $user_id)->where('account_id', $account_id)->where('module_id', $module_id)->delete();
	...
	public function userinfo(Request $request)
    {
		...
		if(isset($UserObj)){		
			...
			$favmenus = UserFavMenu::where('user_id', $UserObj->id)->where('account_id', $UserObj->account_id)->get();
   ...
   public function bookfacility(Request $request)
	...
	public function announcement(Request $request) {
		...
		$list_array =array();
		$lists = AnnouncementDetail::where('user_id',$userid)->where('account_id',$UserObj->account_id)->orderBy('id','DSC')->get();
		$data =array();
	...
	public function inboxMessage(Request $request)
    {
		...
		$messages = InboxMessage::whereIn('type',[2,3,7,8,9,10,11,12,13,14,15])->where('event_status','!=',1)->where('user_id',$input['user_id'])->orderby('id','desc')->get();  
		$data = array();
		if(isset($messages)){
	...
	public function eform_reg_vehicle_file_category(Request $request)
   {
	...
	public function bookinglists(Request $request)
	{
		...
		$bookings = FacilityBooking::where('user_id',$input['user'])->get();
		if(isset($bookings)){
		...
		$bookings = Defect::where('user_id',$input['user'])->get();
		if(isset($bookings)){
		...
		// Key Handover
		$keyhandover = array();
		$bookings = UnittakeoverAppointment::where('user_id',$input['user'])->get();
		if(isset($bookings)){
	...
	public function user_notifications(Request $request)
	{
		...
		// Facility
		$records = array();
		$notifications = UserNotification::where('user_id',$input['user'])->where('account_id',$input['property'])->orderby('id','desc')->get();
