...
use App\Models\v2\AnnouncementNotification;
...
use App\Models\v2\VisitorTypeSubcategory;
...
use App\Models\v2\EformMovingPayment;
use App\Models\v2\EformMovingInspection;
use App\Models\v2\EformMovingDefect;
use App\Models\v2\EformRenovationPayment;
use App\Models\v2\EformRenovationInspection;
use App\Models\v2\EformRenovationDefect;
use App\Models\v2\EformDoorAccesscardPayment;
use App\Models\v2\EformDoorAccesscardAck;
use App\Models\v2\UserFacialId;
use App\Models\v2\Employee;
use App\Models\v2\RoleDevice;
use App\Models\v2\RoleRemoteDevice;
...
use App\Models\v2\QrcodeOpenRecord;
...
use App\Models\v2\FinanceAdvancePayment;
use App\Models\v2\FinancePaymentLog;
use App\Models\v2\FinanceCreditPayment;
use App\Models\v2\UserProperty;
use App\Models\v2\FirebaseNotification;
...
class OpsApiController extends Controller
{
	...
	public function change_password(Request $request) {
		...
		$messages=array(
			'login_id.required' => 'Lopgin Id is missing',
	...
	public function logoutHistoryLogs(Request $request)
    {
		...
		UserManagerLog::where('user_id', $input['user_id'])->where('login_from',$input['login_from'])->where('fcm_token',$input['fcm_token'])->update(['status' => 0]);
				...
				if($group->mobilemodules){
					...
					foreach($group->mobilemodules as $module){
						$group_name = '';
								...
								else if(in_array($module->id,$settings_module)){ 
	...
	public function switchproperty(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if(empty($emp_result)){
				...
				$emp = new \App\Models\v2\Employee();
	...
	public function createannouncement(Request $request) 
    {
		...
		else
		{
			...
			if($input['roles'] !='')
			{
				...
				$emails =array();
				//$users = DB::table('users')->select('id','name','email')->where('account_id',$input['account_id'])->whereIn('role_id',$input['role_array'])->where('account_enabled',1)->get();
				$user_lists = UserPurchaserUnit::where('property_id',$input['account_id'])->whereIn('role_id',$input['role_array'])->groupBy('user_id')->get();
					...
					if(count($user_lists) >0){
						foreach($user_lists as $user_list){
							...
							if(isset($user)){
								$detail['a_id'] = $announcement->id;
								$detail['user_id'] = $user->id;
								$detail['account_id'] = $user_list->property_id;
								$detail['name'] = $user->name;
								$detail['email'] = $user->email;
								$detail['status'] =  0; 
								$detail['created_at'] =  $announcement->created_at;                                
								//$details[] = $detail;
								$result = AnnouncementDetail::create($detail);

								$inbox['account_id'] = $input['account_id'];
								$inbox['unit_no'] = $user_list->unit_id;
								$inbox['user_id'] = $user->id;
								$inbox['type'] = 1;
								$inbox['ref_id'] = $announcement->id;
								$inbox['title'] = $input['title'];
								$inbox['message'] = $input['notes'];
								$inbox['status'] =  0; 
								$inbox['view_status'] =  0;   
								...
								//Start Insert into notification module
									$notification = array();
									$notification['account_id'] = $input['account_id'];
									$notification['user_id'] = $user->id;
									$notification['unit_no'] = $user_list->unit_id;
									$notification['module'] = 'announcement';
									$notification['ref_id'] = $result->id;
									$notification['title'] = $input['title'];
									$notification['message'] = $input['notes'];
								...
								$SettingsObj = UserNotificationSetting::where('user_id',$user_list->user_id)->where('account_id',$user_list->property_id)->first();
								if(empty($SettingsObj) || $SettingsObj->announcement ==1){
									...
									$probObj = Property::find($user_list->property_id);
			                    	$title = "Aerea Home - ".$probObj->company_name;
                                	$message = "New Announcement";
									$notofication_data = array();
									$notofication_data['body'] =$input['title'];
									$notofication_data['unit_no'] =$user_list->unit_id;   
									$notofication_data['user_id'] =$user_list->user_id;   
									$notofication_data['property'] =$user_list->property_id;
									$purObj = UserPurchaserUnit::where('property_id',$user_list->property_id)->where('unit_id',$user_list->unit_id)->where('user_id',$user_list->user_id)->first(); 
									if(isset($purObj))
										$notofication_data['switch_id'] =$user_list->id;
	...
	public function usersummarylist(Request $request) 
    {
		...
		if($adminObj->role_id ==1){
            ...
            $users = UserMoreInfo::whereNotIn('status',[2])->where(function ($query) use ($account_id,$prop_userids) {
                ...
                if($prop_userids !='')
                    $query->orwhereIn('id', $prop_userids);
		...
		foreach($users as $user){
			...
			if(in_array($user->getuser->role_id,$app_user_lists)){
				$moreinfo = new \App\Models\v2\UserMoreInfo();
			...
			$record['name']=isset($user->first_name)?$user->first_name:null;
			$record['last_name']= isset($user->last_name)?$user->last_name:null;
			...
			$record['userinfo']['last_name'] = isset($user->last_name)?$user->last_name:null;
			$record['userinfo']['phone'] = isset($user->phone)?$user->phone:null;
			...
			$record['unit'] = isset($unitinfo->addunitinfo)?"#".$unitinfo->addunitinfo->unit:null;
			...
			if(isset($PurchaserUnits)){
				foreach($PurchaserUnits as $PurchaserUnit){
					...
					$eachunit['unit'] = isset($PurchaserUnit->addunitinfo)?$PurchaserUnit->addunitinfo->unit:null;
	...
	public function searchuser(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($name != '' || $last_name !='' || $role !='' ||$unit !='' ||$building !='' ) {
				$units = array();
				if($unit !=''){     
				...
				$users = UserMoreInfo::whereNotIn('status',[2])->where(function ($query) use ($name,$last_name,$userids,$role,$unit,$units,$app_user_lists,$account_id,$building) {
					if($name !=''){
					...
					if($unit !='' || $building !=''){
							$userids = UserPurchaserUnit::where(function ($subquery) use ($account_id,$units,$building) {
								if($account_id !='')
					...
					if($role !=''){
						if(in_array($role,$app_user_lists)){
							...
							$query->whereIn('user_id',$prop_userids)->where('account_id',$account_id);
						}else{
							$userids = User::where(function ($query) use ($account_id,$role) {
								if($account_id !='')
									$query->where('account_id',$account_id);
								if($role !='')
									$query->Where('role_id', $role);
				...
				foreach($users as $user){
                    ...
                    if(isset($unit) && $unit >0){
                        $unitObj = new \App\Models\v2\Unit();
					...
					$record['role']= isset($unitinfo->role->name)?$unitinfo->role->name:null;
					$record['status']=$user->status;
					$record['created_at']=$user->created_at->format('d/m/Y');
					//$record['name']=$user->name;
	...
	public function userinfo(Request $request) 
    {
		...
		$messages=array(
			'login_id.required' => 'Login id is missing',
			'user.required' => 'User Id is missing',
			'user_info_id.required' => 'User More Info Id is missing',
		);
		...
		if(empty($permission) && $permission->delete!=1){
			...
			if(in_array($UserObj->role_id,$user_roles)){
				...
				if(isset($emp_result)){
                    ...
                    $emp_obj = new \App\Models\v2\Employee();
			...
			if(!in_array($UserObj->role_id,$user_roles)){ //for employee roles
				...
				if(isset($PurchasedUnits)){
					foreach($PurchasedUnits as $PurchasedUnit){
						...
						$record['unit'] = isset($PurchasedUnit->addunitinfo->unit)?$PurchasedUnit->addunitinfo->unit:null;
	...
	public function edituser(Request $request) 
    {
		...
		$permission = $userObj->check_permission(7,$adminObj->role_id); 
		if(empty($permission) && $permission->edit!=1){
			return response()->json(['data'=>null,'response' => 200, 'message' => 'Permission denied']);
		}
		else{
		...
		if(empty($permission) && $permission->create!=1){
			...
			if(in_array($input['role_id'],$user_roles))
			{
				...
				if(in_array($user->role_id,$user_roles))
				{
					...
					if($roomuuids !=''){
						$api_obj = new \App\Models\v2\User();
						$household = $api_obj->household_add_api($thinmoo_access_token, $accountinfo->property_id,$name,$purchaserunit->user_id,$roomuuids);
					}
				...
				/* $api_obj = new \App\Models\v2\User();
					...
					$role_obj = new \App\Models\v2\Role();
					...
					if($role_result['code'] !=0){
						...
						$add_role_result = $role_obj->role_add_api($thinmoo_access_token,$role_data);
	...
	public function userunits(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			$user_data['name']= $UserObj->name." ".$userMoreInfoObj->last_name;
			...
			if($PurchaserUnits){
				foreach($PurchaserUnits as $k => $PurchaserUnit){
					...
					$data['unit'] = isset($PurchaserUnit->addunitinfo)?$PurchaserUnit->addunitinfo->unit:null;
	...
	public function assignunit(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if(in_array($input['role_id'],$user_roles))
			{
				...
				$api_obj = new \App\Models\v2\Unit();
				...
				$name = $userMoreInfoObj->first_name." ".$userMoreInfoObj->last_name;
					...
					if($roomuuids !=''){
						...
						if($household_result['code'] ==0){
							$household = $api_obj->household_modify_api($thinmoo_access_token, $account_id,$name,$UserObj->id,$roomuuids);
						}
						else{
	...
	public function useraccess(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			$modules =array();
			$property = new \App\Models\v2\Property();
		...
		if(empty($permission) && $permission->view!=1){
			...
			if(isset($faceids)){
				foreach($faceids as $faceid){
					...
					$record['unit']= isset($faceid->getunit->unit)?$faceid->getunit->unit:null;
					$record['name']= isset($faceid->user->name)?$faceid->user->name:null;
					...
					$record['face_picture'] = $faceid->face_picture;
					$data[] = $record;
				}
			}
	...
	public function searchfaceid(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($name != '' || $relationship !='' || $unit !='') {				
				...
				$faceids =  UserFacialId::where('account_id',$account_id)->where('status',2)->where(function ($query) use ($relationship,$name,$userids,$unit,$units) {
					if($name !='')
	...
	public function getroleslist(Request $request) 
    {
		...
		$roles = Role::WhereRaw("CONCAT(',',account_id,',') LIKE ?", '%,'.$adminObj->account_id .',%')->orWhere('account_id',0)->get();
	...
	public function getstafflist(Request $request) 
    {
		...
		$staffs = User::where("account_id",$adminObj->account_id)->where("role_id",$role)->orderby('name','asc')->get();
		$data = array();
		if(isset($staffs)){
			foreach($staffs as $staff){
				$result = array();
				$result['id'] = $staff->id;
				$result['name'] = $staff->name.$staff->userinfo->last_name;
	...
	public function uploadstafffaceid(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			return response()->json(['users'=>$emp_result,'response' => 1, 'message' => 'Success']);
	...
	public function createdevice(Request $request) 
    {
		...
		if(empty($permission) && $permission->create!=1){
			...
			if($device->id >0){
				...
				$api_obj = new \App\Models\v2\Device();
	...
	public function searchdevice(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($name != '' || $serial_no != '' || $status !='') {
				...
				return response()->json(['lists'=>$devices,'response' => 1, 'message' => 'Success']);
	...
	public function devicesummarylist(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			foreach($devices as $device){
				...
				$list['status'] = (isset($result['connectionStatus']) && $result['connectionStatus'] ==1)?"Online":"Offline";
	...
	public function cardsummarylist(Request $request) 
    {
		...
		if($adminObj->role_id ==1){
			...
			$units = Unit::where('account_id',$adminObj->account_id)->pluck('unit', 'id')->all();
		}
		
		return response()->json(['cards'=>$cards,'units'=>$units,'response' => 1, 'message' => 'Success']);
		}
	}
	...
	public function unitlist(Request $request) 
    {
		...
		return response()->json(['units'=>$units,'response' => 1, 'message' => 'success']);
	...
	public function searchkeycollection(Request $request) 
    {
		...
		else
		{
			...
			if ($name != '' || $month != '' || $unit != '' || $status != '') {
				...
				})->whereNotIn('unittakeover_appointments.status', ['0'])->get();
				$data = array();
				foreach($units as $unit){
					$record['submission_info']=$unit;
					$record['unit_info'] = $unit->getunit;
	...
	public function defectslist(Request $request) 
    {
		...
		$defects = Defect::where('account_id',$account_id)
                ->orderby('id','desc')
				->get();  
	...
	public function defectsinfo(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			$defect['submission'] = $defectObj->submissions;
	...
	public function defectssearch(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($name != '' || $ticket !='' || $status !='' || $fromdate !='' || $unit !='') {
				$unit = $request->input('unit');
				$unitObj = Unit::select('id')->where('account_id',$account_id)->where('unit',$unit)->get();
				$units = array();
				if(isset($unitObj)){
				...
				$defects =  Defect::select('defects.*')->where('defects.account_id',$account_id)->join('users', 'users.id', '=', 'defects.user_id')->where(function ($query) use ($name,$ticket,$fromdate,$todate,$unit,$units,$status) {
					if($ticket !='')
					...
					if($fromdate!='' && $todate !='')
						$query->whereBetween('defects.created_at',array($fromdate,$todate));
				})->orderby('defects.id','desc')->get();
	...
	public function searchfeedback(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($ticket != '' || $unit != '' || $fromdate != '' || $status != '' || $category != ''|| $filter != '') {
				$feedbacks = FeedbackSubmission::SELECT('feedback_submissions.*')->where('feedback_submissions.account_id',$account_id)->join('users', 'users.id', '=', 'feedback_submissions.user_id')->where(function ($query) use ($ticket,$category,$fromdate,$todate,$unit,$units,$status) {
					if($ticket !='')
						$query->where('feedback_submissions.ticket', 'LIKE', '%'.$ticket .'%');
					if($unit !='')
						$query->wherein('feedback_submissions.unit_no', $units);
					if($status !='')
						$query->where('feedback_submissions.status',$status);
					if($category !='')
						$query->where('feedback_submissions.fb_option',$category);
					if($fromdate!='' && $todate !='')
						$query->whereBetween('feedback_submissions.created_at',array($fromdate,$todate));
	...
	public function facilityinfo(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			$record['user_info'] = $userMoreInfoObj;
				//$record['user_info'] = $booking->getname;
			//$record['user_info'] = $bookingObj->getname;

			return response()->json(['booking'=>$record,'response' => 1, 'message' => 'Success']);
	...
	public function searchfacility(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($unit != '' || $category !='' || $fromdate !='' || $status !='' || $filter !='') {
				$bookings = FacilityBooking::where('account_id',$account_id)->where(function ($query) use ($category,$fromdate,$todate,$unit,$units,$status) {
					if($category !='')
		...
		if(empty($permission) && $permission->edit!=1){
			...
			if($bookingObj->payment_required==1 && $bookingObj->payment_status==2 && isset($bookingObj->opn_charge_id) && $bookingObj->opn_charge_id !=''){
				...
				if(isset($bookingObj->gettype)){
					...
					//echo "Cut of date: ".$cut_of_date;
					//echo " Booking date : ".$bookingObj->booking_date;
					//echo "  -- ";
					if($bookingObj->booking_date < $cut_of_date){
						...
						//echo "Percentage : ".$deduct_percentage;
			...
			for($i=1;$i<=5;$i++){
				...
				if(isset($input[$building_name]) && $input[$building_name] !='') {
					...
					if($building->id >0){
						...
						$api_obj = new \App\Models\v2\Building();
		...
		if(empty($permission) && $permission->view!=1){
			...
			if(isset($units)) {
				foreach($units as $unit){
					$records = array();
					$records['unit'] = $unit;
					$records['building'] = $unit->buildinginfo;
					$data[] = $records;
				}
			}			
			return response()->json(['data'=>$data,'response' => 1, 'message' => 'Success']);
	...
	public function unitsummarysearch(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			return response()->json(['data'=>$units,'response' => 1, 'message' => 'Success']);
		}
	}
	...
	public function facilitytypeinfo(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			$file_path = image_storage_domain();
			return response()->json(['type'=>$facilityObj,'file_path'=>$file_path,'response' => 1, 'message' => 'Success']);
	...
	public function facilitytypecreate(Request $request) 
    {
		...
		if(empty($permission) && $permission->create!=1){
			...
			if(empty($input['payment_required'])){
	...
	public function roleedit(Request $request) 
    {
		...
		if(empty($permission) && $permission->edit!=1){
			...
			$input = $request->all();
			$roleObj = Role::find($id);
			$roleObj->account_id  = $adminObj->account_id;
			...
			$role_obj = new \App\Models\v2\Role();
			$role_result = $role_obj->role_check_record($thinmoo_access_token, $roleObj->account_id,$roleObj->id);

			if($role_result['code'] ==0){  
				$add_role_result = $role_obj->role_modify_api($thinmoo_access_token,$roleObj);
				$role_access = $role_obj->role_access_api($thinmoo_access_token,$roleObj,$device_serial_lists);
			}
			else{        
				$roleObj->parentUuid = 3;
				$add_role_result = $role_obj->role_add_api($thinmoo_access_token,$roleObj->account_id,$roleObj);
				$role_access = $role_obj->role_access_api($thinmoo_access_token,$roleObj,$device_serial_lists);
			}
	...
	public function resFileSearch(Request $request) 
    {
		...
		if(empty($permission) && $permission->view!=1){
			...
			if ($category != '' || $status != '' || $unit != '' || $month != '' ) {
				$submissions =  ResidentFileSubmission::where('account_id',$account_id)
					->where(function ($query) use ($category,$unit,$userids,$month,$from_date,$to_date,$status) {
						if($status !='')
							$query->where('status', $status);
						if($unit !='')
							$query->whereIn('user_id', $userids);
	...
	public function moveinoutsummary(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				foreach ($submissions as $k => $submission) {
					...
					$record['submitted_by'] = isset($submission->user)?$submission->user:null;
	...
	public function moveinoutsearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' || $name != '' || $status != '') {
					$submissions =  EformMovingInOut::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function renovationsearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' || $name != '' || $status != '') {
					$submissions =  EformRenovation::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function dooraccesssearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' || $name != '' || $status != '') {
					$submissions =  EformDoorAccess::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function regvehiclesearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' ||$name != '' ||$status != '' ) {
					$submissions =  EformRegVehicle::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function changeaddresssearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' || $name != '' || $status != '') {
					$submissions =  EformChangeAddress::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function updateparticularsearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($ticket != '' || $unit != '' || $name != ''|| $status != '') {
					$submissions =  EformParticular::where('account_id',$account_id)->where(function ($query) use ($ticket,$unit,$userids,$name,$status) {
						if($status !='' )
							$query->where('status', $status);
						if($unit !='' || $name !='')
							$query->whereIn('user_id', $userids);
	...
	public function visitorsummary(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				foreach ($bookings as $k => $booking) {
					...
					{
						$record['unit'] =  isset($booking->user->userinfo->getunit->unit)?$booking->user->userinfo->getunit->unit:null;
						$record['invited_by'] =isset($booking->user->name)?$booking->user->name:null;
						$record['entry_date'] =  '';
						$record['entry_time'] =  '';
                    }else{
						$record['unit'] = isset($booking->getunit->unit)?$booking->getunit->unit:null;
	...
	public function visitorqrvalidation(Request $request)
    {
		...
		$bookingObj = VisitorBooking::find($bookingId);
		if(empty($bookingObj)){
			return response()->json(['response' => 100, 'message' => 'Booking id not valid']);
		}
	...
	public function visitorsearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($bookingid != '' || $unit != '' || $date != ''|| $purpose != '' || $booking_type != '') {
					$bookings = VisitorBooking::where('account_id',$account_id)->where(function ($query) use ($bookingid,$unit,$userids,$date,$purpose,$booking_type) {
						if($bookingid !='')
		...
		if(empty($permission) && $permission->view!=1){
			...
			if(isset($invoices)){
				foreach($invoices as $invoice){
					$notification = new \App\Models\v2\FinanceInvoice();
	...
	public function dooropen(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				$device = new \App\Models\v2\Device();
	...
	public function callunit(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				foreach($records as $record){
					$result = array();
					$result['record'] = $record;
					$bt = new \App\Models\v2\CallPushRecord();
	...
	public function qropenrecords(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				foreach($records as $record){
					$result = array();
					$result['record'] = $record;
					$bt = new \App\Models\v2\QrcodeOpenRecord();
					$rec = $bt->deviceDetail($record->devSn);  
					$result['device_info'] = isset($rec)?$rec:null;
	...
	public function unitsummary(Request $request) 
    {
		...
		if($type == 7 && isset($user_permission) && $user_permission->view==1){
			...
			$data = array();
			foreach($userMoreInfoObjs as $userMoreInfoObj){
					$user = User::find($userMoreInfoObj->user_id);
					$record['id']=$user->id;
					$record['name']=$userMoreInfoObj->first_name;
					$record['last_name']=isset($userMoreInfoObj->last_name)?$userMoreInfoObj->last_name:null;
					$record['email']=$user->email;
					$record['account_enabled']=$user->account_enabled;
					$record['role_id']=$user->role_id;

					if(in_array($user->role_id,$app_user_lists)){
						$moreinfo = new \App\Models\v2\UserMoreInfo();
						$unit_unit_info = $moreinfo->userunitinfo($userMoreInfoObj->user_id,$unit);
						//print_r($unit_unit_info);
						$record['role'] = isset($unit_unit_info->role->name)?$unit_unit_info->role->name:null;
						$record['role_id'] = isset($unit_unit_info->role_id)?$unit_unit_info->role_id:null;
						//$record['building_name'] = isset($unitinfo->addubuildinginfo)?$unitinfo->addubuildinginfo->building:null;
						//$record['unit'] = isset($unitinfo->addunitinfo)?"#".$unitinfo->addunitinfo->unit:null;
					 }
					else 
						$record['role']=isset($user->role->name)?$user->role->name:null;
			...
			foreach($keycollections as $keycollection){
				...
				$record['unit_info'] = $keycollection->getunit;
				$record['user_info'] = $keycollection->getname;
				$data[] = $record;
			}
			return response()->json(['data'=>$data,'response' => 1, 'message' => 'Success']);
		}
	...
	public function update_notification(Request $request)
	{
		...
		$messages=array(
			'login_id.required' => 'Login id is missing',
			'login_id.required' => 'Notification id is missing',
		);
