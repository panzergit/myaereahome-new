...
class StaticController extends Controller
{
	...
	public function submit_registration(Request $request)
    {
		if($request){
            ...
            if ($request->file('profile') != null) $input['profile_picture'] = $request->file('profile')->store('user_request');
            if ($request->file('contract') != null && $input['role_id']==29) $input['contract_file'] = $request->file('contract')->store('user_request');
