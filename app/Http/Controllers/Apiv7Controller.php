...
class Apiv7Controller extends Controller
{
	public function test_firebase(Request $request){
		...
		$device_tokens = array();
		$device_tokens[] = 'erOlV-GrqU9ivuHi3KWRjd:APA91bHEsq5VramjMDm88Tj_wlcs0KNDNimHg9N0347Hb1G-USZM7i41DzNG-lWDKVgKd1G_UnWr78cyjJFLNOQKLxdaWx9P3p03YoDlm88szDQB8J_61JjsIr-pqyy-BmQ4n6_l9Gsr';
		$fields = [
...
*/
	...
	public function verifyLoginApi(Request $request) {
        ...
        if (Auth::attempt($credentials)) {
			...
			if($user && $user->status !=1){
				...
				$name = $user->name;
				LoginOTP::sendotpnew($name, $email,2);
	...
	public function getunitlist(Request $request) 
    {
		...
		//$userMoreInfoObj = UserMoreInfo::where('user_id',$userObj->id)->where('account_id',$userObj->account_id)->where('status',1)->first(); 
	...
	public function visitingPurpose(Request $request)
   {
	...
	public function inspectionsingnature(Request $request) {
		...
		if($defectObj->status !=1 && $defectObj->inspection_status ==1 ){ //ticket not closed
   ...
   public function MpAdsSubmit(Request $request)
	...
	public function chatAttachmentAPI(Request $request) {
		...
		$messages=array(
			'user_id.required' => 'Sender id missing',
	...
	public function encryptstring(Request $request) 
    {
		...
		$messages=array(
			'user_id.required' => 'Login id is missing',
