...
class OpsApiv3Controller extends Controller
{
	...
	public function login(Request $request) {
        ...
        if (Auth::attempt($credentials)) {
			...
			if($user->role_id ==1){
				...
				$name = $user->name;
				LoginOTP::sendotpnew($name, $email,3);
				$data =array();
	...
	public function searchuser(Request $request) 
    {
		...
		if(empty($permission) ){
			...
			if ($name != '' || $last_name !='' || $role !='' ||$unit !='' ||$building !='' ) {
				...
				$users = UserMoreInfo::whereNotIn('status',[2])->where(function ($query) use ($name,$last_name,$userids,$role,$unit,$units,$app_user_lists,$account_id,$building,$firstname_userids,$lastname_userids) {
					if($name !=''){
	...
	public function defectDashboard(Request $request)
	{
		...
		$messages=['user_id.required' => 'User id missing'];
		$validator = Validator::make($request->all(), $rules, $messages);
		if ($validator->fails()) {
            $messages = $validator->messages();
            $errors = $messages->all();
	    ...
	    $account_id = User::find($request->user_id)->account_id;
        ...
        $defactsByLocations = DefectLocation::where('account_id',$account_id)->where('status',1)
            ->when($locations,fn($q) => $q->whereIn('id', $locations))
        ...
        $defactsByTypes = DefectType::where('account_id',$account_id)->where('status',1)
            ->when($types,fn($q) => $q->whereIn('id', $types))
        ...
        $defactsByUnits = Unit::where('account_id',$account_id)->where('status',1)
            ->when($units,fn($q) => $q->whereIn('id', $units))
	   ...
	   return response()->json(['ac' => $account_id, 'data'=>$data,'response' => 1, 'message' => 'Success!']);
	}
	...
	public function defectssearch(Request $request) 
    {
		...
		if(empty($permission) ){
			...
			if ($name != '' || $ticket !='' || $status !='' || $fromdate !='' || $unit !='') {
				...
				$defects =  Defect::select('defects.*')->where('defects.account_id',$account_id)->where(function ($query) use ($name,$userids,$ticket,$fromdate,$todate,$unit,$units,$status) {
					if($ticket !='')
	...
	public function searchfeedback(Request $request) 
    {
		...
		if(empty($permission) ){
			...
			if ($ticket != '' || $unit != '' || $fromdate != '' || $status != '' || $category != ''|| $filter != '') {
				$feedbacks = FeedbackSubmission::where('account_id',$account_id)->where(function ($query) use ($ticket,$category,$fromdate,$todate,$unit,$units,$status) {
					if($ticket !='')
	...
	public function resFileSearch(Request $request) 
    {
		...
		if(empty($permission) ){
			...
			if ($category != '' || $status != '' || $unit != '' || $month != '' ) {
				...
				->where(function ($query) use ($category,$unit,$units,$month,$from_date,$to_date,$status) {
					if($status !='')
						$query->where('status', $status);
					if($unit !='')
						$query->whereIn('unit_no', $units);
	...
	public function visitorsearch(Request $request) 
	{
			...
			if(empty($permission) && $permission->view!=1){
				...
				if ($bookingid != '' || $unit != '' || $date != ''|| $purpose != '' || $booking_type != '') {
					$bookings = VisitorBooking::where('account_id',$account_id)->where(function ($query) use ($bookingid,$unit,$units,$date,$purpose,$booking_type) {
						if($bookingid !='')
