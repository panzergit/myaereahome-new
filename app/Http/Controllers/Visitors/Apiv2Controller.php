	...
	public function visitingPurpose(Request $request)
   {
	...
	public function visitorRegisitration(Request $request)
    {
		...
		if($propObj->security_option == 2){
			...
			//$inviteurl = env('MAIN_URL')."/opslogin/visitor-summary/".$booking->id."/manualscan";
        ...
        for($i=1;$i<=$slot_available;$i++){
            ...
            if(!empty($request->input($name)) && !empty($request->input($mobile))){
				...
				if($propObj->security_option != 2){
					...
					if($propObj->security_option == 3 && 1==2)
                    	$inviteurl = env('MAIN_URL')."/opslogin/visitor-summary/".$bookingObj->id."/facialscan/";
					else
						$inviteurl = '';
					$qr_content = $inviteurl."bid=".$bookingObj->ticket."&vid=".$visitorObj->id;
					//$inviteurl = env('MAIN_URL')."/opslogin/visitor-summary/".$bookingObj->id."/edit";
					//$qr_content = "bid=".$bookingObj->ticket."&vid=".$visitorObj->id;
					$qrsize = 350;
					$img =  base64_encode(QrCode::format('png')->size($qrsize)->errorCorrection('H')->margin(5)->generate($qr_content));
					$qrdata = base64_decode($img);
					$randnum = rand(10000,99999).'-'.date('YmdHis');
					...
					$visitorObj->qrcode_file = $randnum.'.png';
					$visitorObj->save();      
				...
				if($propObj->security_option != 2)
					...
					$emailObj = new \App\Models\v7\VisitorBooking();
					$result = $emailObj->qrcode_email($bookingObj->id, $bookingObj->user_id, $data['name'],$data['email'],$data['mobile'],$data['vehicle_no'],$qrcodefile_email,$data['id_number']);
            }
		...
		$inbox['type'] = 14;
		$inbox['ref_id'] = $bookingObj->id;
		$inbox['title'] = "Visitor Registration";
		$inbox['message'] = '';
		//$inbox['reno_date'] = $reno->reno_date;
		$inbox['status'] =  0; 
		$inbox['view_status'] =  0; 
		$inbox['created_at'] =  $bookingObj->created_at;   
		...
		$probObj = Property::find($UserObj->account_id);
			if($probObj->manager_push_notification ==1){ //if push notification activated for manager app
				...
				//$message = "You have been received new visitor registration";
	...
	public function visitorRegSummary(Request $request){
		...
		foreach($records as $k => $record){
			...
			$data[$k]['invited_by'] = isset($record->user->name)?$record->user->name:'Walk-In';
	...
	public function visitorBookingInfo(Request $request){
		...
		if(empty($record)){
			return response()->json(['response' => 200, 'message' => 'Booking not found']);
		}
		//$data[ =$record;
		if(isset($record->visitors))
			$record['visitors'] = $record->visitors;
	...
	public function getqrstatus(Request $request){
		...
		*/
		...
		$deviceObj = Device::where('account_id',$bookingObj->account_id)->where('device_serial_no',$device)->first();
		if(empty($deviceObj)){	
			return response()->json(['code' => 99999, 'msg' => 'Permission Denied']);
		}
		...
		if(isset($bookingObj) && $bookingObj->id>0)
		{
			...
			if(empty($device_info)){
				return response()->json(['code' => 99999, 'msg' => 'Device not available in community']);
			}
			//echo $bookingObj->unit_no;
			$unitObj = Unit::find($bookingObj->unit_no);
			if(empty($unitObj)){
				return response()->json(['code' => 99999, 'msg' => 'Unit not available in community']);
			}
			
			$locations = explode(",",$device_info->locations);
			//print_r($locations);
			if(!in_array($unitObj->building_id,$locations))
			{
				return response()->json(['code' => 99999, 'msg' => 'Permission denied']);
			}
	...
	public function trailgetqrstatus(Request $request){
		...
		*/
		...
		$device = $input['devSn'];
		$bookingObj = VisitorBooking::where('ticket',$ticket)->first();
		$deviceObj = Device::where('account_id',$bookingObj->account_id)->where('device_serial_no',$device)->where('status',1)->first();
		if(empty($deviceObj)){	
			return response()->json(['code' => 99999, 'msg' => 'Permission Denied']);
		}
