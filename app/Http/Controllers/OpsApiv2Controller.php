...
use App\Models\v2\ActivityLog;


use App\Models\v2\AndroidManagerVersion;
use App\Models\v2\IosManagerVersion;

class OpsApiv2Controller extends Controller
{
	...
	public function forgotPassword(Request $request) {
		...
		$roles = explode(",",$env_roles);
		if(empty($user)){
			$data = response()->json(['response' => 3, 'message' => 'Email not registered!']);
		}
		else if(in_array($user->role_id,$roles)){
	...
	public function testadminmenu(Request $request) {
		$rules=array(
			'login_id' => 'required',
		);
		$messages=array(
			...
			if(isset($staffs)){
				foreach($staffs as $staff){
						...
						if(isset($staff_moreinfo))
                        	$result['name'] = $staff->name." ".$staff_moreinfo->last_name;
                    	else
	...
	public function searchkeycollection(Request $request) 
    {
		...
		else
		{
			...
			if ($name != '' || $month != '' || $unit != '' || $status != '') {
				...
				*/
				$units =  UnittakeoverAppointment::where('unittakeover_appointments.account_id',$account_id)->where(function ($query) use ($name,$unit,$units,$month,$from_date,$to_date,$status) {
					if($status !='')
						$query->where('unittakeover_appointments.status',$status);
					if($unit !='')
						$query->whereIn('unittakeover_appointments.unit_no', $units);
	...
	public function roleedit(Request $request) 
    {
		...
		if(empty($permission) && $permission->edit!=1){
			...
			foreach($devices as $device) {
				...
				//echo $request->input($device_checked);
				if($request->input($device_checked) ==1)
					{                   
						//echo $device->device_serial_no;
	...
	public function unitsummary(Request $request) 
    {
		...
		if($type == 7 && isset($user_permission) && $user_permission->view==1){
			...
			foreach($userMoreInfoObjs as $user){
					...
					$record['last_name']=isset($user->last_name)?$user->last_name:null;
					$record['phone']=isset($user->phone)?$user->phone:null;
					...
					$record['unit_assigned_details']=$PurchaseUnitDetail;
