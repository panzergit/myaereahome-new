...
class Device extends Model
{
    ...
    protected $fillable = [
        'account_id','device_name','device_serial_no','locations','model','device_type','position_type','proximity_setting','status','facility_type','entry_allowed_in_advance','start_time','end_time'
    ];

    public function buildinginfo(){
        return $this->belongsTo('App\Models\v7\Building','locations');
    }
    ...
    public function getLocations($location){
        ...
        $locations = Building::select('building','id')->whereIn('id',$locations)->orderby('building','asc')->get();
       return $locations;
        ...
        public function device_add_api($token,$device) {
            ...
            if($device->device_type ==2){
                $fields = [
                    ...
                    'positionType'          => '0' //Shared door access                
                ];
            }else{
                $fields = [
                    'accessToken'           =>  $token,
                    ...
                    'deviceModelname'       =>  $device->model,
                    'positionUuids'         =>  $device->locations,
                    'positionType'          =>  1, // for Ordinary door access 
                ];
            }
    ...
    public function device_batch_add($token,$device,$useruuids) {
        $url = env('THINMOO_API_URL')."persEmpHousehold/extapi/designatedDeviceAuthByDevSn";
        //The data you want to send via POST
            $fields = [
                'accessToken'           =>  $token,
                'extCommunityUuid'      =>  $device->account_id,
