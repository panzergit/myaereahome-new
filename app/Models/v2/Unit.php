...
class Unit extends Model
{
    ...
    protected $fillable = [
        'account_id','building_id','unit','code','size','share_amount'
    ];

    public function takeover_appt(){
        return $this->hasOne('App\Models\v2\UnittakeoverAppointment','unit_no','id')->where('status','=', 1)->orwhere('status','=', 2);
    }
    ...
    public function buildinginfo(){
        return $this->belongsTo('App\Models\v2\Building','building_id');
    }

    public function unit_info($user,$unit,$building =''){
        ...
        $record = UserPurchaserUnit::where('user_id',$user)->where(function ($subquery) use ($units,$building) {
            if($building !='')
