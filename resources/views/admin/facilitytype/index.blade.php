                    ...
                    <div class="row">
                      <div class="col-lg-4">
                         <div class="form-group row">
                               <a href="{{url("/opslogin/configuration/facility/create")}}"  class="submit mt-4 ml-3 float-left" style="width:auto"> + Add New</a>
                           </div>
                       </div>
                    </div>
                    @endif
                     <thead>
                        <tr>
                           <th>#</th>
                           <th>facility </th>
                           <th>calendar availability (days)</th>
                           <th>show slot availability (days)</th>
                           <th>next booking </th>
                           <th>actions</th>
                        </tr>
                     </thead>
                      ...
                      @if($facilities)
                       ...
                       @foreach($facilities as $k => $facility)
                        <tr>
                           ...
                           <td  class="spacer">{{$facility->facility_type}}</td>
                           <td  class="spacer">{{$facility->calendar_availability_start}}</td>
                           <td  class="spacer">{{$facility->allowed_booking_for}}</td>
                           <td  class="spacer">@php 
                           if($facility->next_booking_allowed ==1)
                              echo "None";
                           else if($facility->next_booking_allowed ==2)
                              echo "Month";
                           else if($facility->next_booking_allowed ==3)
                              echo $facility->next_booking_allowed_days." Day(s)";
                           ...
                           <a class="dropdown-item" href="{{url("opslogin/configuration/facility/$facility->id/edit")}}">Edit</a>
                           @endif
                           @if(isset($permission) && $permission->delete==1)
                           <a class="dropdown-item" href="#" onclick="delete_record('{{url("opslogin/configuration/facility/delete/$facility->id")}}');" >Delete</a>
                           @endif
                                    </div>
                                 </div>
