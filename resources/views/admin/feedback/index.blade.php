...
@php 
   ...
   $permission = $permission->check_permission(26,$permission->role_id); 
@endphp
...
<div class="row">
               ...
               <ul class="summarytab">
                     ...
                     @if(isset($feedback->view) && $feedback->view==1 && $admin_id !=1)
                        <li  class="activeul"><a href="{{url('/opslogin/configuration/feedback#feedbacksettings')}}">Feedback  <br>Options</a></li>
                     @endif
                    ...
                    <div class="row">
                      <div class="col-lg-4">
                         <div class="form-group row">
                               <a href="{{url("/opslogin/configuration/feedback/create")}}"  class="submit mt-4 ml-3 float-left" style="width:auto"> + Add New</a>
                           </div>
                       </div>
                    </div>
                    @endif
                     ...
                     <tbody>
                      @if($feedbacks)
                       ...
                       @foreach($feedbacks as $k => $dept)
                        <tr>
                           <td  class="roundleft">{{$k+1}}</td>
                           <td  class="spacer">{{$dept->feedback_option}}</td>
                           ...
                           <a class="dropdown-item" href="{{url("opslogin/configuration/feedback/$dept->id/edit")}}">Edit</a>
                           @endif
                           @if(isset($permission) && $permission->delete==1)
                           <a class="dropdown-item" href="#" onclick="delete_record('{{url("opslogin/configuration/feedback/delete/$dept->id")}}');">Delete</a>
                           @endif
                                    </div>
                                 </div>
