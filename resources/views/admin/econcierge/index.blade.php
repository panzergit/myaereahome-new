                    ...
                    <div class="row">
                      <div class="col-lg-4">
                         <div class="form-group row">
                               <a href="{{url("/opslogin/configuration/econcierge/create")}}"  class="submit ml-3 mt-2 float-left" style="width:auto"> + Add New</a>
                           </div>
                       </div>
                    </div>
                     @endif
                     ...
                     <tbody>
                      @if($econcierges)
                        ...
                        @foreach($econcierges as $k => $econcierge)
                        <tr>
                           <td  class="roundleft">{{$k+1}}</td>
                           <td>@if(!empty($econcierge->banner_image))
                                       <div class="image-uploeconcierge">
                                          <label for="file-input">
                                          <img src="{{$file_path}}/{{$econcierge->banner_image}}" class="viewimg"/>
                                          </label>
                                       </div>
                                    @endif</td>
                           <td  class="spacer">{{$econcierge->banner_title}}</td>
                           <td  class="roundright">
                            <div class="dropdown">
                                 ...
                                 <a class="dropdown-item" href='{{url("opslogin/configuration/econcierge/$econcierge->id/edit")}}'>Edit</a>
                           
                              @if($econcierge->status ==0)
                                 <a class="dropdown-item" href="#" onclick="activate_record('{{url("opslogin/configuration/econcierge/activate/$econcierge->id")}}');" >Deactive</a>
                              @else
                                 <a class="dropdown-item" href="#"  alt="De-Activate" title="De-Activate"  onclick="deactivate_record('{{url("opslogin/configuration/econcierge/deactivate/$econcierge->id")}}');">Active</a>
                              @endif
                              
                              <a class="dropdown-item" href="#" onclick="delete_record('{{url("opslogin/configuration/econcierge/delete/$econcierge->id")}}');" >Delete</a>
                                    </div>
                                 </div>
